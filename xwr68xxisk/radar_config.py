...
class TriggerModeCommand(RadarCommand):
    ...
    @property
    def mode(self) -> int:
        """Get trigger mode (0: timer-based, 1: software, 2: hardware)"""
        return self.params[0]
    
    @mode.setter
    def mode(self, value: int):
        """Set trigger mode (0: timer-based, 1: software, 2: hardware)"""
        if value not in [0, 1, 2]:
    ...
    def is_timer_based(self) -> bool:
        """Check if trigger mode is timer-based"""
        return self.mode == 0
    
    @property
    def is_software_trigger(self) -> bool:
        """Check if trigger mode is software"""
        return self.mode == 1
    
    @property
    def is_hardware_trigger(self) -> bool:
        """Check if trigger mode is hardware"""
        return self.mode == 2
