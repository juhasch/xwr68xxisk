...
from xwr68xxisk.radar_config_models import (
    RadarConfig, AntennaConfigEnum, DfeDataOutputModeConfig, ModeType,
    ChannelConfig, AdcConfig, AdcBits, AdcOutputFormat, AdcBufConfig, ProfileConfig
)
from enum import Enum

def create_default_radar_config(**kwargs):
    """Helper function to create a RadarConfig with default required fields."""
    defaults = {
...
def test_basic_config_generation():
    ...
    config = create_default_radar_config()
...
def test_gui_monitor_config():
    ...
    assert "guiMonitor -1 1 1 1 1 1 1" in cfg_str_all
    
    # Test with no plots enabled
    config_no_plots = create_default_radar_config(
        plot_scatter=False,
    ...
    assert "guiMonitor -1 0 0 0 0 0 0" in cfg_str_none
...
def test_chirp_config():
    ...
    # Check for TX configurations (only TX1 and TX3 are used with mask 5)
    assert "chirpCfg 0 0 0 0 0 0 0 1" in cfg_str  # TX1
    assert "chirpCfg 1 1 0 0 0 0 0 4" in cfg_str  # TX3
...
def test_unsupported_antenna_config():
    ...
    # The config generator should still generate a valid config with default values
    assert "% Profile generated from GUI" in cfg_str
    assert "channelCfg 15 5 0" in cfg_str  # Default channel config
    assert "chirpCfg 0 0 0 0 0 0 0 1" in cfg_str  # Default chirp config
...
def test_profile_config_format():
    ...
    config = create_default_radar_config(
        ...
        profile_cfg=ProfileConfig(
            profile_id=0, start_freq=60.0, idle_time=7.0, adc_start_time=3.0, ramp_end_time=24.0,
            freq_slope_const=166.0, tx_start_time=1.0, num_adc_samples=256, dig_out_sample_rate=12500,
            hpf_corner_freq1=0, hpf_corner_freq2=0, rx_gain=158.0
        )
    )
    cfg_str = generate_cfg_from_scene_profile(config)
